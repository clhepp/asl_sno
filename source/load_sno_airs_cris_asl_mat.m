function [s] = load_sno_airs_cris_asl_mat(sdate1, sdate2, xchns, res)
%
...
%           (i) date1: first month as string: 'YYYY/MM/DD'
%           (ii) date2: last month as string:  'YYYY/MM/DD'
%              N.B. Only accepts the same year.
%           (iii) xchns: numeric IDs of CrIS channels to load based on NO guard channel list. 
%              eg [403 499 737 884 905 998 1021 1297] or [399:408], 
%              LW: [1:713], MW [714:1146], SW [1147:1305]
%           (iv) res: source. one of: {'low', 'high'}.
%
...
dp     = ['/home/chepplew/data/sno/airs_cris/ASL/LR/' cyr1 '/'];
...
[~, dchns] = intersect(fa2c_trim, xf);                        % xj: maps fc onto fa2c

% the A2C channels are a subset of the CrIS channels
[iwant,~] = seq_match(fc_2g(cchns), fa2c_trim(dchns));
...
for ifn = ifn1:2:ifn2;
  ...
  if( ismember('nbr_rLW', {vars.name}) & ismember('nbr_ra', {vars.name}) & ...
    ...
    if  (size(rc,1) == 1317 & size(ra,1) == 2645 & size(ra2c,1) == 1178)    % or 1185
      s.ra      = [s.ra,   ra(achns,:)];                   % 
      rc_ham    = single(hamm_app(double(rc)));
      s.rc      = [s.rc,   rc_ham(cchns(iwant),:)];      clear rc_ham;

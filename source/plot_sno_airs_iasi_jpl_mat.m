% plot_sno_airs_iasi_jpl_mat.m
%
% Working with data supplied by: allchns_sno_airs_cris_jpl_mat.m
%  [ sa qn ] = allchns_sno_airs_iasi_jpl_mat('2013/01/01', 1);
...
% Prep some data:
igrp  = 1;
bands = [650, 1140; 1210, 1615; 2182 2550];
...
% ---------- Global Stats Spectrum w/truncation of spikes, gaps shaded ---------------
j = igrp;
figure(1);clf;plot(fa(achns),abm,'-',fa(achns),dbm,'g-');grid on;xlim([bands(igrp,:)]);
figure(2);clf;plot(fa(achns),abm - dbm,'-');grid on;axis([bands(igrp,:),-2 2]);
%
trunc_bias   = qn.dbm - qn.abm;
trunc_stderr = qn.stderr;
tr_ind =  find(qn.stderr >= 0.01);
trunc_stderr(tr_ind) = 0.01;
tr_ind = find(qn.dbm - qn.abm >= 0.4);     trunc_bias(tr_ind) = 0.4;
tr_ind = find(qn.dbm - qn.abm <= -0.4);    trunc_bias(tr_ind) = 0.4;

run airs_filled_gaps                 % ./data/airs_filled_gaps.m -> fa_gap [10x2]
junk = sort(reshape(fa_gap,1,20));
figure(3);clf;[hax,hl1,hl2]=plotyy(sa.fa(sa.achns),trunc_bias,sa.fa(sa.achns), trunc_stderr);
  grid on;xlim(hax(:),[bands(igrp,:)]);ylim(hax(1),[-1 1]);ylim(hax(2),[0 0.06]);
  hax(1).YTick = [-0.8:0.2:0.8]; hax(2).YTick = [0:0.005:0.02];
  xlabel('wavenumber cm^{-1}');ylabel(hax(1),'Mean Bias K');ya2=ylabel(hax(2),'Std. error K');
  set(ya2, 'Units', 'Normalized', 'Position', [1.05, 0.7, 0]);
  title('2013 AIRS L1b IASI SNO LW Bias'); legend('bias','std. error','Location','north');
  %aslprint([phome 'AI_SNO_2013_biasStdErr_LW_wGaps.png']);

figure(3);hold on;
for i = 1:10 hlin(i) = line([junk(i) junk(i)],[-1 1]); end
gray = [0.5 0.5 0.5];
pa1 = patch([fa_gap(1,1) fa_gap(1,2) fa_gap(1,2) fa_gap(1,1)], [-1 -1 1 1],gray);
patch([fa_gap(2,1) fa_gap(2,2) fa_gap(2,2) fa_gap(2,1)], [-1 -1 1 1],gray);
patch([fa_gap(3,1) fa_gap(3,2) fa_gap(3,2) fa_gap(3,1)], [-1 -1 1 1],gray);
patch([fa_gap(4,1) fa_gap(4,2) fa_gap(4,2) fa_gap(4,1)], [-1 -1 1 1],gray);
...
% -------------- HISTOGRAMS: for 900 wn channel:  ----------------- %
  ...
  figure(4);clf;hold on;plot(btbinx,c403_pdf,'+-');plot(btbinx,d403_pdf,'o-');grid on;
